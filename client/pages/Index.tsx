...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-[80px] md:pt-[100px]">
      ...
      {/* Hero Section */}
      <section className="relative px-6 md:px-12 lg:px-[72px] pt-24 md:pt-32 lg:pt-[175px] pb-20 md:pb-28 lg:pb-[140px]">
        <motion.h1
        ...
        {/* CTA Buttons */}
        <div className="mt-16 md:mt-20 lg:mt-[107px] flex flex-col sm:flex-row items-center gap-4 md:gap-5">
          <button onClick={scrollToTeam} className="inline-flex h-[60px] px-12 justify-center items-center gap-2 rounded-[20px] border border-white bg-transparent relative hover:bg-white/10 transition-colors">
            <svg
      ...
      >
        ...
        <div className="max-w-[889px] mx-auto space-y-6 md:space-y-8 lg:space-y-10">
          ...
          <TeamCard
            ...
            description="A Computer Science graduate with a strong passion for storytelling, branding, and connecting technology with people. As Marketing Lead at Aseer, Batool crafts compelling narratives that reflect the company's vision, communicate its purpose, and build deep trust with users
 She believes that great technology becomes powerful only when people can understand it, feel it, and connect with it."
          />
...
function TeamCard({ name, title, description, descriptionSize = "text-base" }: TeamCardProps) {
  return (
    <motion.div
      className="w-full min-h-[275px] rounded-[15px] border border-white p-6 md:p-8 lg:p-10 flex flex-col cursor-pointer"
      style={{
        background: "linear-gradient(90deg, #6F3EFF 0%, #000 100%)",
        boxShadow: "0 10px 30px rgba(111, 62, 255, 0.2)",
      }}
      initial={{ opacity: 0, y: 20 }}
      ...
      whileHover={{
        boxShadow: "0 20px 60px rgba(111, 62, 255, 0.6)",
      }}
    >
